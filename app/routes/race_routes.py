...
async def get_race_details(race_id: int, spark: SparkSession = Depends(get_spark)):
    """
...
async def get_race_results(race_id: int, spark: SparkSession = Depends(get_spark)):
    """
    경주 결과를 조회하는 엔드포인트
...
async def get_fastest_laps(race_id: int, spark: SparkSession = Depends(get_spark)):
    """
